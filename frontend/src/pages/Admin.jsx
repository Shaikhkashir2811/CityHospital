...
const StatusBadge = ({ done }) => (
  <span className={`inline-flex items-center gap-1.5 text-[9px] font-black uppercase tracking-widest px-2.5 py-1 rounded-full ${done ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'
    }`}>
...
═══════════════════════════════════════════════════════════════════ */
const AppointmentRow = ({ item, idx, onDismiss, onDelete, showDate, showActions }) => (
  <tr
  ...
  >
    ...
    {/* Date — only for upcoming / history */}
    {showDate && (
      <td className="px-6 py-4">
        <div className="inline-flex items-center gap-1.5 bg-blue-50 text-blue-600 text-[11px] font-bold px-3 py-1.5 rounded-full">
          <Calendar size={10} />
      ...
      <td className="px-6 py-4">
        <div className="flex items-center gap-2">
          <button
            ...
            className={`w-8 h-8 rounded-xl flex items-center justify-center transition-all duration-200 ${item.isCompleted
              ? 'cursor-pointer hover:scale-105 active:scale-95'
              : 'bg-slate-100 text-slate-300 cursor-not-allowed opacity-40'
              }`}
            style={item.isCompleted ? {
              background: 'linear-gradient(135deg,#34d399,#059669)',
              boxShadow: '0 2px 8px rgba(52,211,153,.35)', color: '#fff'
            } : {}}
          >
          ...
          <button
            ...
            disabled={item.isCompleted}
            title={item.isCompleted ? 'Cannot delete completed' : 'Remove appointment'}
            className={`w-8 h-8 rounded-xl flex items-center justify-center transition-all duration-200 ${!item.isCompleted
...
═══════════════════════════════════════════════════════════════════ */
const AppointmentCard = ({ item, idx, onDismiss, onDelete, showDate, showActions }) => (
  <div
  ...
  >
    ...
    <div className="flex flex-wrap items-center gap-2 mb-4">
      ...
      {showDate && (
        <div className="inline-flex items-center gap-1.5 bg-blue-50 text-blue-600 text-[11px] font-bold px-2.5 py-1.5 rounded-lg">
          <Calendar size={10} />
          {formatSlotDate(item.slotDate)}
        </div>
      )}
    ...
    {showActions && (
      <div className="flex items-center gap-2 pt-3 border-t border-slate-100">
        <button
          onClick={() => { if (item.isCompleted) onDismiss(item._id); }}
          disabled={!item.isCompleted}
          className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-xl text-xs font-bold transition-all duration-200 ${item.isCompleted
        ...
        >
          <CheckCircle size={13} strokeWidth={2.5} /> Mark Done
        </button>
        <button
          onClick={() => { if (!item.isCompleted) onDelete(item._id, item.name); }}
          disabled={item.isCompleted}
...
const AdminDashboard = () => {
  ...
  const appointments = useMemo(() => {
    ...
    const visible = filtered.filter(a => !dismissedIds.has(a._id));
    if (!search.trim()) return visible;
    const q = search.toLowerCase();
    return visible.filter(a =>
      a.name?.toLowerCase().includes(q) || a.email?.toLowerCase().includes(q)
    );
  }, [dashData, search, dismissedIds, queueFilter]);
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="relative" style={{ animation: 'fadeUp .5s ease .2s both' }}>
            ...
            <div className="absolute pointer-events-none hidden sm:block"
              style={{
                top: '-60px', left: '-80px', width: 320, height: 320, borderRadius: '50%',
                background: 'radial-gradient(circle,rgba(52,211,153,.13) 0%,transparent 65%)', zIndex: 0
              }} />
            <div className="absolute pointer-events-none hidden sm:block"
              style={{
                bottom: '-70px', right: '-60px', width: 360, height: 360, borderRadius: '50%',
            ...
            <div className="relative z-10 bg-white rounded-3xl overflow-hidden"
              ...
              {/* Card header */}
              <div className="relative px-4 sm:px-7 py-4 sm:py-5 flex flex-col gap-3 overflow-hidden">
                  ...
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    ...
                    <div>
                      <h2 className="text-sm font-black text-slate-900 leading-none">
                        {queueFilter === 'today' ? "Today's Queue"
                          : queueFilter === 'upcoming' ? 'Upcoming Appointments'
                            : 'Past History'}
                      </h2>
                ...
                {/* Filter tabs */}
                <div className="relative flex gap-1.5 bg-slate-50 rounded-xl p-1 w-full sm:w-fit">
                  {[
                    ...
                    <button
                      key={key}
                      onClick={() => { setQueueFilter(key); setSearch(''); }}
                      className={`flex-1 sm:flex-none px-4 py-2 rounded-lg text-xs font-black uppercase tracking-wider transition-all duration-200 ${queueFilter === key
                ...
                <thead>
                  <tr>
                    {[
                      'Patient',
                      'Doctor',
                      ...(queueFilter !== 'today' ? ['Date'] : []),
                      'Slot',
                      'Status',
                      ...(queueFilter !== 'history' ? ['Actions'] : []),
                    ].map(h => (
                ...
                <tbody>
                  {appointments.length === 0 ? (
                      ...
                      <td colSpan={6} className="px-6 py-16 text-center">
                        <div className="flex flex-col items-center gap-3">
                          ...
                          <p className="text-sm font-bold text-slate-400">
                            No {queueFilter === 'today' ? "appointments for today" : queueFilter === 'upcoming' ? "upcoming appointments" : "past appointments"}
                          </p>
                    ...
                    <AppointmentRow
                      ...
                      showDate={queueFilter !== 'today'}
                      showActions={queueFilter !== 'history'}
                    />
                  ))}
                </tbody>
              </table>
